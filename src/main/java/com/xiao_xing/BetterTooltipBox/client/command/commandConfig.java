                ...
                if (itemStack != null) {
                    if (ConfigManager.Instance.SpecialItemList.containsKey(args[1])) {
                        ...
                        if (list.contains(
                            ...
                            sender
                                .addChatMessage(new ChatComponentText("成功添加!请在所有物品添加完成后使用 /btooltipbox save 来保存配置文件"));
                            list.add(
                                Objects.requireNonNull(GameRegistry.findUniqueIdentifierFor(itemStack.getItem())) + ":"
                                    + itemStack.getItemDamage());
                            return;
                    ...
                    if (ConfigManager.Instance.SpecialItemList.containsKey(args[1])) {
                        ...
                        if (list.contains(
                            ...
                            list.remove(
                                Objects.requireNonNull(GameRegistry.findUniqueIdentifierFor(itemStack.getItem())) + ":"
                                    + itemStack.getItemDamage());
                            TextureManager.INSTANCE.removeSpecialItem(
                                Objects.requireNonNull(GameRegistry.findUniqueIdentifierFor(itemStack.getItem())) + ":"
                                    + itemStack.getItemDamage());
                            sender
                                .addChatMessage(new ChatComponentText("成功移除!请在所有物品移除完成后使用 /btooltipbox save 来保存配置文件"));
                            return;
                        }
                    }
